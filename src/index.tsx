...
function BrushChart({
  ...
  const filterStock = (stock, transformMatrix) => {
    const { translateX } = transformMatrix;
    const start = Math.max(Math.min(stock.length - Math.floor(translateX / 20), 0), stock.length);
    const ret = stock.slice(start, 50);
    debugger;
    return ret;
  }
  ...
  const constrainZoom = (transformMatrix) => ({
    ...transformMatrix,
    translateX: Math.min(Math.max(transformMatrix.translateX, 0), (280 - 50) * scaleFactor)
  })
